...
- Deploy changes or publish container images
...
5. Review security-sensitive changes manually, including the `Dockerfile` and the GitHub Actions workflows.
...
- Is compatible with the project's `MIT` license
- Includes any required attribution
- Does not reproduce copyrighted or incompatibly licensed material

Because `MIT` is permissive rather than copyleft, the practical risk runs one way: code carrying a
copyleft licence such as `GPL` or `AGPL` cannot be absorbed into this project, even though `MIT` code
may be used almost anywhere else. Treat AI-suggested material of unknown provenance as unusable
rather than assuming a permissive licence makes it safe.
